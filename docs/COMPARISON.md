# Lume vs Python, JavaScript/Node, Go, Rust, Mojo

An honest comparison. Lume is **v0.2 — early but real**: the core language, a
...
Lume is the right tool* and, just as importantly, when it is not.

The short version: Lume's bet is **the AI-native surface built into the
...
more mature. Where Lume wins, it wins on **time-to-first-AI-feature and a single
...
| | **Lume** | **Python** | **JS / Node** | **Go** | **Rust** | **Mojo** |
|---|---|---|---|---|---|---|
...
- **Lume's genuine wins** are the leftmost rows: `ai()`, JSON, an HTTP client,
...
- **Where the others win** — and they do: Go, Rust, and Node's V8 are **faster**
  than Lume (Node JITs; Go and Rust compile to optimized machine code). All five
  are **far more mature**, run on **every platform**, and have **enormous
  ecosystems**. Lume has none of that yet, runs on **Apple Silicon only**, and
  has **no JIT and no GC** today.
- Lume's HTTP is marked "via curl" on purpose: `http`/`ai` shell out to the
...
| Workload | Lume v0.2 | CPython 3.13 | Node (V8 JIT) |
|---|---:|---:|---:|
| `fib(30)` | 0.097 s | 0.094 s | 0.017 s |
| tight loop to 50M | 3.0 s | 4.3 s | — |

Lume is **on par with CPython** on recursion and **~1.4× faster** on tight
numeric loops, and **~5× faster** than its own first tree-walking interpreter.
**V8 is faster** on `fib` because it JIT-compiles hot code; **Lume does not JIT
...
interpolation, first-class functions, closures, lists and maps. What Lume
...
maturity, and the ability to run anywhere. Lume is Apple-Silicon-only for now.
...
reach. Lume is synchronous and single-binary, which is simpler but less powerful
...
networked services today; Lume is the faster path to an AI script or prototype.
...
Lume has no GC *and* no ownership model yet (it frees its arena at exit), so it
...
hardware). Lume's bet is **AI-native glue**: calling models, parsing their
JSON, and doing semantic search with zero setup. What you'd **give up** choosing
Lume: Mojo's serious numeric performance and its Python-superset
interoperability. What Lume gives you instead: a tiny, dependency-free,
MIT-licensed binary where `ai("...")` is a keyword-level convenience, not a
library import. Different problems — Mojo makes the math fast; Lume makes the
LLM call trivial.

---

## When NOT to use Lume (yet)

Be honest with yourself. Pick something else if:

- **It's production-critical.** Lume is v0.2. There's no GC yet (the heap is
...
- **You depend on a large ecosystem.** No NumPy, no pandas, no web framework, no
  ...
  deliberately small. If your project lives on third-party libraries, Lume can't
  serve it yet.
...
Where Lume **does** fit right now: AI scripts and prototypes, glue between a
...
to summarize it.** This is the exact workflow Lume was built to remove the
friction from.
...
**What Lume removed:** a virtual environment, two `pip` dependencies (`requests`,
...
and digging the text back out of `message.content[0].text`. In Lume the HTTP
call, the JSON parse, and the LLM call are three built-in calls and the model's
answer comes back as a plain string.

**What Python keeps that Lume doesn't have (yet):** explicit timeouts and
...
verbose because it's more configurable and more mature. Lume trades that
configurability for a near-zero-setup path to the same result.

---

*Every Lume claim here is backed by the code, the test suite, and the honest
