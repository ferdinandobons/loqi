...
| tight loop to 50M | 3.0s | 4.3s | n/a |
...
- **Young and small-ecosystem.** v0.2: no package registry, no NumPy/pandas
  equivalent, the standard library is what ships in the binary.

What it is *not* missing: memory is reclaimed during execution by a precise
mark-sweep garbage collector (leak-clean and clean under AddressSanitizer/UBSan),
and multi-file programs work through namespaced `import`. These are deliberate
trade-offs for an early, fast-moving project, not hidden costs: the binary is one
file, and the language is being shaped while the runtime is hardened.
